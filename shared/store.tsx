...
export function Provider({children}:{children:React.ReactNode}) {
 ...
 useEffect(()=>{AsyncStorage.getItem(STORE_KEY).then(s=>{if(s){const data=JSON.parse(s);if(data?.entries&&data?.batteries)setState({...initialState,...data});}}).catch(()=>setStorageError('Saved data could not be loaded. Changes remain in memory.')).finally(()=>setReady(true));},[]);

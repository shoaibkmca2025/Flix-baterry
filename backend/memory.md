...
Last updated: 25 September 2026 · Owner: 4AM Global Media (Vaibhav Pasi, Co-Founder) · Client: Felix Batteries Industries, Nashik
...
| D-12 | The plate/model catalogue | **closed 2026-09-25** — the client's warranty grid, 38 products. The code before the model number is the **plate count as a letter: its position in the alphabet** (G=7 … M=13 … W=23), verified against the client's earlier plate sheet (26 of 27 rows). Tubular uses two-character series codes (SE, S5, ME, SG, BE, MG, SS), all 30 months. `GP` in front = the Gold Power brand (red case), always 12 months. Seeded in `seed/masters.ts`; the pre-catalogue rows are inactive but still resolvable. **One row still open:** the 400 at 18 months is seeded as `H 400` (8 plates) because the alphabet rule says so — the client's sheet writes `I 400` at both 18 and 24 months, and the earlier sheet wrote 9P twice. Flip the seed if he confirms `I`. | closed (one row assumed) | — |
| D-13 | Battery identity | **closed 2026-09-25** — the factory restarts the 4-digit serial at 1 on the **26th of every month** and counts **separately per product**, so `M1000 26090001` and `S1000 26090001` are two different batteries. The 8 digits are therefore NOT unique: a battery is identified by its **full printed label** — product code + digits — stored in `batteries.battery_code` (`domain/serials.fullCode`). Every lookup needs the product: from the label's own prefix, or from the plates+model the dealer picked. A lookup that misses reports `otherProductsWithTheseDigits` so a wrong pick is caught rather than creating a twin. | closed | — |

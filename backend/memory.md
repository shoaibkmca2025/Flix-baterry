...
Last updated: 26 September 2026 · Owner: 4AM Global Media (Vaibhav Pasi, Co-Founder) · Client: Felix Batteries Industries, Nashik
...
1. Dealer scans the old battery's code at the counter (camera) → app extracts `mfgMonth` (first 4 digits, `YYMM`) and `serialNo` (last 4 digits), and the plate letter + model number if the label carries the prefix (`M2200-…`) — else the dealer picks them from two dropdowns → server checks warranty (D-11: manufacture month + the (plate, model) term + grace; a chained battery uses its chain's dates) → blocks the flow if expired.
...
## 2. Current state (26 Sep 2026)

| Area | State |
|---|---|
| Dealer app UI | Complete against the client HTML; works in a browser; camera/GPS/signature untested on real phones. **Connected to the backend**: sign-in/registration, the capture flow (d10–d17, live old-battery lookup on d11), My requests/history (server audit trail), dispatch (d33 → `claims.dispatch`), Sync now (sends drafts, re-hydrates). Photo evidence still local-only (D-10 open). d37 credit notes still derived locally (amounts match the server's demo rates). |
| Admin UI | Complete; desktop + phone layouts. **Connected to the backend** via the store-hydration bridge (`src/api/mapping.ts` + `sync.ts`): dealers, entries (+claims), batteries (+chains), stock ledger, audit log, admin accounts all come from the server after sign-in; approve/reject (two-step: entry, then claim once the old battery is received/checked), dealer lifecycle, Record an entry, returns stages, manual stock movements and account management call the API. Corrections, review/void, overrides, policies, catalogue and serial import are guarded as "not available in this version". |
...
| D-03 | Warranty anchor and term | ~~Original sale date, 24 months for every model~~ | **superseded 2026-09-22 by D-11** (closed with the client 17 Sep, then replaced by the client's own plate/model rule). What survives: every replacement is recorded in a chain (old serial → new serial) and the chain **never restarts** — a replacement inherits the first battery's dates unchanged. | — |
...
| D-11 | Warranty rule: plate × model term, counted from manufacture | The label reads **plate letter + model number** (`M2200` = M plates, model 2200; the 8-digit code follows: `M2200-26041212`). Each (plate, model) combination is a `battery_models` row carrying its **term in months** (client's examples: M2200 = 30, N2200 = 24). Cover runs from the **first day of the manufacture month** in the code, for the term **plus a grace** (`settings warranty.grace_months`, default 2) because a battery may sit up to two months between manufacture and sale: a 24-month model made April 2026 is covered 2026-04-01 → 2028-05-31. Dealer picks plate and model from two dropdowns (pre-filled from a prefixed label). Because no sale record is needed, an old battery **not on record** can be replaced: `entries.approve` puts it on record (`notOnRecord`) with a chain from its manufacture month and the plate/model the dealer named, then judges it like any other. | **closed 2026-09-22** — client insight relayed by the team; anchor/term/label questions confirmed with the team the same day | — |
| D-12 | The plate/model catalogue | **closed 2026-09-25/26** — the client's warranty grid, **37 products**. The code before the model number is the **plate count as a letter: its position in the alphabet** (G=7 … M=13 … W=23), verified against his earlier plate sheet (26 of 27 rows). Tubular uses two-character series codes (SE, S5, ME, SG, BE, MG, SS), all 30 months. `GP` in front = the Gold Power brand (red case), always 12 months. The 400 range was settled by the client on 25 Sep: **`G 400` = 12, `I 400` = 18, and the 24-month 400 is withdrawn** (`H 400` retired). Seeded in `seed/masters.ts`, which is the source of truth — re-running it updates terms and retires anything he removes; retired rows stay resolvable for the batteries that reference them. | closed | — |
| D-13 | Battery identity | **closed 2026-09-25** — the factory restarts the 4-digit serial at 1 on the **26th of every month** and counts **separately per product**, so `M1000 26090001` and `S1000 26090001` are two different batteries. The 8 digits are therefore NOT unique: a battery is identified by its **full printed label** — product code + digits — stored in `batteries.battery_code` (`domain/serials.fullCode`). Every lookup needs the product: from the label's own prefix, or from the plates+model the dealer picked. A lookup that misses reports `otherProductsWithTheseDigits` so a wrong pick is caught rather than creating a twin. | closed — but see D-15 | — |
| D-15 | Which batch month does a battery get stamped with? | The serial restarts on the **26th** while D-14 says the stamp is the month of manufacture. If the stamp rolls on the 26th too (a battery made 28 Sep printed `2610`, so `2609` = 26 Aug–25 Sep), each stamp period holds exactly one serial cycle and product+YYMM+serial can never repeat — the identity built in D-13 is then sufficient and nothing changes. If instead the stamp follows the calendar month, one month spans two serial cycles and two batteries could print the same identity, which the app would refuse as a duplicate. **Working assumption: the stamp rolls on the 26th** (the team's reading, and the only one consistent with a restarting serial). Cover is anchored at the 1st of the stamped month either way; a battery made 26–31 of the previous month therefore gets a few days' extra cover, which the 2-month grace absorbs. | **open** — one question to the client: *is a battery made on 28 September printed 2609 or 2610?* | before go-live |
| D-16 | A chain keeps the numbers it was opened with | `warranty_chains` stores `term_months` and `grace_months` **separately** (they used to be merged, and the term was re-derived by subtracting whatever the grace setting said *today* — so changing the grace would have silently misreported every existing chain). A chain is permanent while the catalogue and the grace can both change, so it carries its own term, its own grace, and `expiry_before_override`. Migration `0011` split the existing rows; no cover dates changed. | closed 2026-09-26 | — |
| D-17 | Warranty overrides | A battery whose cover has ended can still be replaced if head office says so — the dealer app has always said "Request an admin override before submitting", and until 26 Sep 2026 no such thing existed, so the message was empty and the dealer was stuck. `warranty_overrides` + `POST /warranty/overrides` (dealer asks, bounded by `settings warranty.override_max_days`, default 90), `…/approve` and `…/reject` (`warranty.override.decide`). Approving is the **only** path that moves a chain's expiry after a sale; it records what the expiry was. One request in flight per chain. | closed 2026-09-26 | — |
| D-14 | What the YYMM in the code means | The serial restarts on the **26th**, so the month stamp probably rolls on the 26th too — `2609` may mean 26 Aug–25 Sep or 26 Sep–25 Oct rather than calendar September. Cover is anchored at the **1st of the stamped month**, which the 2-month grace absorbs in the customer's favour either way. | **open** — ask the client | before go-live |

...
| D-15 | Can two batteries share product + YYMM + serial? | The serial restarts on the **26th** while the stamp is the **calendar month** (D-14), so a month's production spans the tail of one serial cycle and the start of the next — e.g. an `M 1000` made 10 Sep and another made 28 Sep are both stamped `2609` and could both be numbered `0001`. That would be two batteries with the same printed identity, and the app would reject the second as a duplicate. | **open** — asked the client 26 Sep 2026 | before go-live |
